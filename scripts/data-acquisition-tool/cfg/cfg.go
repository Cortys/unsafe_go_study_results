...
func selectDecl(selectedPkg *packages.Package, selectedFile *ast.File, lineNumber int, matches [][]int) ast.Decl {
	...
	for _, decl := range selectedFile.Decls {
		...
		if matches != nil {
			...
			lineToDeclDist := base.IntervalDistance(lineNumber, declPos.Line, declEnd.Line)
			foundMatch := false
			for _, match := range matches[matchesOffset:] {
				if match[0] >= declPos.Offset && match[1] <= declEnd.Offset {
					matchesOffset++
					foundMatch = true
				} else if foundMatch {
					break
		...
		} else if declPos.Line > lineNumber {
			break
		}
...
func GetCFG(query *base.CFGQuery, projectsDir string)  {
	...
	cgoAstMap := make(map[string]*ast.File)
		...
		if strings.HasPrefix(srcStr, "// Code generated by cmd/cgo") {
			...
			if matches != nil {
				if origSrc, err := ioutil.ReadFile(matches[1]); err == nil {
					ast, _ := parser.ParseFile(fset, filename, src, parser.ParseComments)
					lock.Lock()
					cgoFileMap[filename] = matches[1]
					cgoAstMap[filename] = ast
					lock.Unlock()
					filename = matches[1]
					src = origSrc
				}
